...
for f in files:
    ...
    #print(lines)
    tree = ET.parse("../1102/full/Annotations/"+oneline[0]+".xml")
    root = tree.getroot()
    for child in root.findall('object'):
        ...
        image = cv2.imread(image_path+oneline[0]+".jpg")
    for i in range(lines):        
        ...
        if name == 'Red':
            ...
            color = (18, 153, 255)        
        image = cv2.rectangle(image,(int(float(xmin+cropx1)),
                                     int(float(ymin+cropy1))),
                                    (int(float(xmax+cropx1)),
                                     int(float(ymax+cropy1))), color, 2)
    cv2.imwrite(draw_path+oneline[0]+".jpg", image)